    ...
    : QWidget(parent)
{
    ...
    openButton->setIcon(style()->standardIcon(QStyle::SP_FileDialogContentsView));
...
void Player::metaDataChanged()
{
    if (m_player->isMetaDataAvailable()) {
        setTrackInfo(QString("%1 - %2").arg(m_player->metaData(QMediaMetaData::AlbumArtist).toString()).arg(m_player->metaData(QMediaMetaData::Title).toString()));
        if (m_coverLabel) {
...
void Player::showColorDialog()
{
    if (!m_colorDialog) {
        ...
        QPushButton *button2 = new QPushButton(tr("Reset"));
